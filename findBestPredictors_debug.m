...
if exist('S02_MedData.mat', 'file') ~= 2
    error('findBestPredictors:MissingData',...
        'Unable to find required data file S02_MedData.mat')
else
...
if isfield(X, 'MedData')
    ...
    error('findBestPredictors:BadTable', ...
        'Unable to find table "MedData" in MAT-file.')
end % if else isfield
...
% We know that it's a cell array.
nVars = size(Xtab, 2);
varPairs = cell(nchoosek(nVars, 2), 2); % Number of pairs - by - 2
